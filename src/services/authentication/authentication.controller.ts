...
class AuthenticationController implements Controller {
  ...
  private initializeRoutes(): void {
    ...
    this.router.post(`${this.path}/early-access`, validationMiddleware(CreateUserDto), addUserAgent, this.earlyAccess);
  ...
  /**
   * Collects email for early access to products offering
   */

  private earlyAccess = async (request: RequestWithUser, response: Response, next: NextFunction) => {
    const userData: CreateUserDto = request.body;

    try {
      const data: any = await this.authenticationDao.earlyAccess(userData, request.userAgent);
