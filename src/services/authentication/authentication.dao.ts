...
import EarlyAccessDto from "../../services/user/earlyaccess.dto";
import { Role } from "../user/role.entity";
import { EarlyAccess } from "../user/earlyaccess.entity";
...
class AuthenticationDao implements Dao {
  ...
  public login = async (loginData: UserLoginDto, userAgent: object): Promise<object | Error> => {
    ...
    // a Repository is used to manage CRUD operations on the database
  ...
  public earlyAccess = async (userData: CreateUserDto, userAgent: object): Promise<User | Error> => {
    ...
    if (await userRepository.findOne({ email: userData.email, archived: false })) {
      throw new UserExistsException(userData.email);
    } else {
      try {
        const guestRole = roleRepository.create({ id: "guest"});
        const savedGuest = await roleRepository.save(guestRole);
        const user = userRepository.create({
          ...userData,
          roles: [savedGuest],
        });

        const newEarlyAccessUser: User = await userRepository.save(user);

        // log event to central handler
        const ended: number = Date.now();
        event.emit(ActivityType.CREATE, {
          actor: {id: "System", type: ActorType.Application},
          object: {...newEarlyAccessUser, type: ObjectType.Person},
          resource: this.resource,
          timestamp: ended,
          took: ended - started,
        ...
        await this.notifyByEmail(newEarlyAccessUser, NotificationType.EARLYACCESS);

        logger.info(`User with email ${newEarlyAccessUser.email} just signed up for early access!`);
        return newEarlyAccessUser;
      } catch (error) {
        logger.error(`error dey ${error}`);
        throw new Error("Dem inna inna inna, Dem inna investigate me");
      }
    }
  }
  ...
  private notifyByEmail = async (
    ...
    try {
      ...
      switch (type) {
        case NotificationType.EARLYACCESS:
          emailToken = await createEmailToken(user.email, "1h", TokenTypes.EARLYACCESS);
          emailSubject = `HeyBuddy Early Access Here`;
          emailBody = `Hey amazing human, this is Jeff Ighalo, CTO of HeyBuddy.\
                      I have to say a big thank you for signing up for early access, and i tell you this,\
                      this is going to be the best thing you did for your romantic/personal life.\
                      I promise to make your life easier and richer, just give me a minute to finish the app.`;
          break;
